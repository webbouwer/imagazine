...
class imagazine_login_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'imagazine_login_widget', // Base ID
			__('Imagazine Login Widget', 'imagazine'), // Widget name and description in UI
			array( 'description' => __( 'Imagazine Frontend Login', 'imagazine' ), )
		);
	}
...
if (!$user_ID) { // is not logged in
		...
		echo '<ul class="tabmenu"><li class="signintab"><span >'.__( 'Sign in', 'imagazine' ).'</span></li>';

		// sign-up link or button
		if ( $regallowed ) {

			echo '<li class="registertab"><span >'.__( 'Register', 'imagazine' ).'</span></li>';
		}
		echo '</ul>';
		...
		if ($register == true && $regallowed) {
			...
			echo '<h3>'.__( 'Success!', 'imagazine' ).'</h3>';
			echo '<p>'.__( 'Check your email for the password and use it to sign in', 'imagazine').'</p>';
			...
			// show login elements
			echo '<h3>'.__( 'Sign in', 'imagazine' ).'</h3>';
		}

		// display login form
		wp_login_form();

		echo '<div class="resetlogin"><span>'.__( 'Forgot password?', 'imagazine' ).'</span></div>';
		...
		echo '<h3>'.__( 'Register', 'imagazine' ).'</h3>';
		echo '<p>'.__( 'Sign up', 'imagazine' ).'</p>';
		?>
			<form method="post" action="<?php echo site_url('wp-login.php?action=register', 'login_post') ?>" class="wp-user-form">
			<div class="username">
			<label for="user_login"><?php __('Username', 'imagazine' ); ?>: </label>
			<input type="text" name="user_login" value="<?php echo esc_attr(stripslashes($user_login)); ?>" size="20" id="user_login" tabindex="101" />
			</div>
			<div class="password">
			<label for="user_email"><?php _e('Your Email', 'imagazine' ); ?>: </label>
			<input type="text" name="user_email" value="<?php echo esc_attr(stripslashes($user_email)); ?>" size="25" id="user_email" tabindex="102" />
			</div>
			<div class="login_fields">
			<input type="submit" name="user-submit" value="<?php _e('Sign up', 'imagazine' ); ?>" class="user-submit" tabindex="103" />
			<?php do_action('register_form'); ?>
			<?php if (isset(  $_GET['register'] )) { $register = $_GET['register']; } if($register == true) { echo '<p>'.__( 'Check your email for the password!', 'imagazine' ).'</p>'; } ?>
		...
		<li class="tab3 tab" style="display:none">
			...
			<?php
			echo '<h3>'.__( 'Reset password', 'imagazine' ).'</h3>';
			echo '<p>'.__( 'Reset your password. You\'ll receive an email with link to the reset form.', 'imagazine').'</p>';
			?>

			<form method="post" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>" class="wp-user-form">
			<div class="username">
			<label for="user_login" class="hide"><?php _e('Username or Email', 'imagazine' ); ?>: </label>
			<input type="text" name="user_login" value="" size="20" id="user_login" tabindex="1001" />
			</div>
			<div class="login_fields">
			<input type="submit" name="user-submit" value="<?php _e('Reset my password', 'imagazine' ); ?>" class="user-submit" tabindex="1002" />
			<?php do_action('login_form', 'resetpass'); ?>
			<?php if (isset(  $_GET['reset'] )) { $reset = $_GET['reset']; } if($reset == true) { echo '<p>'.__( 'Check your mailbox for a link to the password reset form.', 'imagazine' ).'</p>'; } ?>
...
<?php } else { // is logged in
			...
			*/
			...
			echo '<li class="menu-item"><a class="logout-link" href="'.wp_logout_url( 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'] ).'" title="Sign off"><span>'.__('Sign off', 'imagazine').'</span></a></li>';
